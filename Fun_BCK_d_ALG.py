...
def Fun_directly_solve( subset_job_index, subset_job_num, release_time_mu, release_time_delta, jobs_num, process_time, current_m_index, Gamma, DT):
    ...
    current_release_time  =  release_time_mu
        # 释放时间从小到大排
    ...
    for j in subset_job_index:   # 只有确定性问题才可以这样子做！
    ...
    if h_t_u > DT: # print(f'子问题不可行')
        sp_status  = 1
            
    return sp_status, h_t_u, J_plus, J_equal, stand_time
...
def Fun_warm_start(machines_num, jobs_num, process_time, Gamma, release_time_mu, release_time_delta, DT):
    global y
...
def BCK_d_main(jobs_num, machines_num, jobtabu, cost, process_time, release_time_mu,
    ...
    cons4 = mp_Model.addConstrs( y[m,j] <= x[m] for m in range(machines_num)for j in range(jobs_num))

    warm_cut_pool, warm_time = Fun_warm_start(machines_num, jobs_num, process_time, Gamma, release_time_mu, release_time_delta, DT)

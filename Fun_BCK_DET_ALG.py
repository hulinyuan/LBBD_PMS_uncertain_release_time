            ...
            if sp_status == 1: # 子问题不可行
                ...
                if cb_cuts[4] == 1:  # print(f'添加割：{sp_status}')
                    ...
                    for j_hat in J_equal:
                        vi = eta[m_idx] - stand_time * y[m_idx, j_hat] - gp.quicksum( process_time[m_idx, j] * y[m_idx,j] for j in J_hat_1) - gp.quicksum( (release_time_mu[j] + process_time[m_idx, j] - stand_time ) * y[m_idx,j] for j in J_hat_2 )
                        Valid_Inequality.append(vi)
...
def BCK_DET_main(jobs_num, machines_num, jobtabu, cost, process_time, release_time_mu,
                  release_time_delta, Gamma, DT, cb_cuts, max_iter, max_time):
    global eta, x, y, DT1, cb_cuts1, release_time_delta1, release_time_mu1, process_time1, Gamma1, jobs_num1, machines_num1
    ...
    eta = mp_Model.addVars(m_index, lb=0 , vtype=GRB.CONTINUOUS,name="eta")
    Tar = mp_Model.addVars(m_index, lb=0 , vtype=GRB.CONTINUOUS,name="Tar")
    # set objective
    mp_Model.setObjective(gp.quicksum(Tar[m]for m in range(machines_num)), GRB.MINIMIZE)
    ...
    cons4 = mp_Model.addConstrs( Tar[m] >= eta[m] - DT for m in range(machines_num))
    # Add constraint
    # set  parameters
    ...
    if mp_Model.status == GRB.Status.INFEASIBLE:
        ...
        opt_e = mp_Model.getAttr("X", eta)
        opt_t = mp_Model.getAttr("X", Tar)
        rel_tm = sp_tm     
        rel_num = sp_num
        itr_num = mp_Model.getAttr(GRB.Attr.NodeCount) 

    return status, obj_val, opt_x, opt_t, opt_e,  opt_y, gap, total_tm, mp_tm, sp_tm, mip_tm, rel_tm, gre_tm, itr_num, sp_num, int_num, fra_num, mip_num, rel_num, gre_num, bk_cuts, bc_cuts
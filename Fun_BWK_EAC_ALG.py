    ...
    while stop_flag == 0:
        ...
        if case_index == 3:
            ...
            if len(J_minus) == 0:
                ...
                for j in subset_job_index:
                    if release_time_mu[j] > release_time_mu[min_j]:
                        job_hat.append(j)  
            
                sp_status, h_t_u, J_equal, J_plus, stand_time, cur_rt_u = Fun_iter_update_method(job_hat, len(job_hat), release_time_mu, release_time_delta, jobs_num, process_time, m_idx, Gamma, DT)
...
def Fun_warm_start(machines_num, jobs_num, process_time, Gamma, release_time_mu, release_time_delta, DT, cb_cuts):
    ...
    return warm_cut_pool, warm_time
        ...
        if subset_job_num > 0: 
            ...
            if h_t_u > DT: 
                ...
                if defici_delta > 0: #
                    ...
                    if h_t_u < DT: # 如果下界小于DT，则需要进一步进行计算
                        mip_start = time.time()
...
def BWK_EAC_main(jobs_num, machines_num, jobtabu, cost, process_time, release_time_mu,
                  release_time_delta, Gamma, DT, cb_cuts, cbw_cuts, max_iter, max_time):
    ...
    warm_cut_pool, warm_time = Fun_warm_start(machines_num, jobs_num, process_time, Gamma, release_time_mu, release_time_delta, DT, cbw_cuts)
